...
export default function RaceResults() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        const data = await getQualiResults(season, round);
        setResults(data);
        ...
        <table className="w-full text-center">
          <thead>
            <tr className="bg-red-700/30 backdrop-blur-md">
              ...
              <th className="p-3">Q1 Time</th>
              <th className="p-3">Q2 Time</th>
              <th className="p-3">Q3 Time</th>
            </tr>
          </thead>
          <tbody>
            {loading ? (
